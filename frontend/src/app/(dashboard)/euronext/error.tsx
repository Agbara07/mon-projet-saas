...
export default function EuronextError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => { console.error('[euronext-error]', error) }, [error])
  return (
    <div className="flex flex-col items-center justify-center h-full gap-4 text-center p-8">
      <AlertTriangle size={32} className="text-red-500" strokeWidth={1.5} />
      <h2 className="text-sm font-semibold text-gray-800">Erreur de chargement Euronext</h2>
